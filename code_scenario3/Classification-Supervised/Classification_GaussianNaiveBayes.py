...
def saveScore(accuracy, detectionRate, far, tt, size_train, size_test):
    ...
    filename  = "Classification_GaussianNaiveBayes.txt"
...
def plot(features, labels):
    notAttack = labels!=1
    attack = labels==1
    plt.scatter(features[notAttack, 1], features[notAttack, 31],c=labels[notAttack],cmap="Set3", label='normal')
    plt.scatter(features[attack, 1], features[attack, 31],c=labels[attack],cmap="Pastel1", label='attack')
    plt.xlabel('Flow Duration')
    plt.ylabel('Packet Length Mean')
    plt.legend()
    finalfig = "Classification_GaussianNaiveBayes.png"
    os.chdir('../figure')
