...
attack = {'BENIGN': 0,'DDoS': 1}

def import_data():
    os.chdir('../../data/MachineLearning')
    with open('Friday-WorkingHours-Afternoon-DDos.pcap_ISCX.csv') as csvfile:
...
def process():
    ...
    #labels for each cluster formed
    label_names = list(map(
        lambda x: pd.Series([train_labels[i] for i in range(len(clf.labels_)) if clf.labels_[i] == x]),
        range(k)))
    ...
    saveScore(k,DR,FAR, TP, FP, TN, FN, accuracy, confusion, classification_r, hamming, matthews, zero_one,
...
def saveScore(k,DR, FAR, TP, FP, TN, FN, accuracy, confusion, classification_r, hamming,matthews, zero_one, tt, size_train, size_test):
    os.chdir('../../result/scenario3/text')
    filename  = "K_mean.txt"
    with open(filename, 'w') as f:
        print('Results: \n', file=f)
        print('Pour k=: '+ str(k)+ ' on a: \n', file=f)
